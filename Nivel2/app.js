...
//Estado de inicio y fin y obstaculo, 0 incio, 1 meta, 3 perro
boardMatrix[0][0] = 0;
boardMatrix[3][2] = 1;
boardMatrix[0][2] = 3;
boardMatrix[3][0] = 3;
...
function dibujarMatriz() {
    ...
    ctx.drawImage(img1, 0 * CELL_HEIGHT + CELL_HEIGHT/8, 0 * CELL_WIDTH+ CELL_HEIGHT/8);
    ctx.drawImage(img2, 2 * CELL_HEIGHT + CELL_HEIGHT/8, 3 * CELL_WIDTH+ CELL_HEIGHT/8);
    ctx.drawImage(img3, 2 * CELL_HEIGHT + CELL_HEIGHT/8, 0 * CELL_WIDTH+ CELL_HEIGHT/8);
    ctx.drawImage(img3, 0 * CELL_HEIGHT + CELL_HEIGHT/8, 3 * CELL_WIDTH+ CELL_HEIGHT/8);
    ...
    try {
        ...
        if(boardMatrix[x1][y1]==1)
        {
            ...
            if(direcciones.length>posiciones.length)
            {
                ...
                localStorage.setItem("nivel2", 0);
                return false;
            }
            else{
                localStorage.setItem("nivel2", 1);
...
function mandarRespuestaCorrectaBD() {
    ...
    db.collection('PuntajeGlobal').doc().set({ nivel:2, aprobado:true})
  }

  function mandarRespuestaIncorrectaBD() {

    db.collection('PuntajeGlobal').doc().set({ nivel:2, aprobado:false})
}
...
function mandarPuntajeBD() {
    ...
    db.collection('PuntajeIndividual').doc().set({docUsuario: localStorage.getItem("documento"), nombreUsuario: localStorage.getItem("usuario"), nivel1:parseInt(localStorage.getItem("nivel1")),nivel2:parseInt(localStorage.getItem("nivel2")),nivel3:0,nivel4:0,nivel5:0,puntaje:parseInt(localStorage.getItem("puntaje"))})
    setTimeout(function(){
