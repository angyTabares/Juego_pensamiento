...
function dibujarMatriz() {
    ...
    ctx.drawImage(img1, 5 * CELL_HEIGHT , 5 * CELL_WIDTH);
    ctx.drawImage(img2, 2 * CELL_HEIGHT , 0 * CELL_WIDTH);
    ctx.drawImage(img3, 3 * CELL_HEIGHT , 2 * CELL_WIDTH);
    ctx.drawImage(img3, 0 * CELL_HEIGHT , 3 * CELL_WIDTH);
    ctx.drawImage(img3, 4 * CELL_HEIGHT , 4 * CELL_WIDTH);
    ctx.drawImage(img3, 1 * CELL_HEIGHT , 5 * CELL_WIDTH);
    ctx.drawImage(img6, 2 * CELL_HEIGHT + CELL_HEIGHT/50, 3 * CELL_WIDTH + CELL_WIDTH/2);
    ctx.drawImage(img6, 1 * CELL_HEIGHT + CELL_HEIGHT/50, 1 * CELL_WIDTH + CELL_WIDTH/2);
    ctx.drawImage(img6, 5 * CELL_HEIGHT + CELL_HEIGHT/50, 3 * CELL_WIDTH + CELL_WIDTH/2);
    ctx.drawImage(img6, 4 * CELL_HEIGHT + CELL_HEIGHT/50, 0 * CELL_WIDTH + CELL_WIDTH/2);
    ...
    try {
        ...
        if(boardMatrix[x1][y1]==1)
        {
            ...
            if(direcciones.length>posiciones.length)
            {
                ...
                localStorage.setItem("nivel4", 0);
                return false;
            }
            else{
                localStorage.setItem("nivel4", 1);
...
function mandarRespuestaCorrectaBD() {
    ...
    db.collection('PuntajeGlobal').doc().set({ nivel:4, aprobado:true})
}
  
function mandarRespuestaIncorrectaBD() {
  
      db.collection('PuntajeGlobal').doc().set({ nivel:4, aprobado:false})
  }
  ...
  function mandarPuntajeBD() {
    ...
    ,nivel2:parseInt(localStorage.getItem("nivel2")),nivel3:parseInt(localStorage.getItem("nivel3")),nivel4:parseInt(localStorage.getItem("nivel4")),nivel5:0,puntaje:parseInt(localStorage.getItem("puntaje"))})
    setTimeout(function(){
